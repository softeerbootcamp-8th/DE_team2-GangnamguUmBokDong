...
.PHONY: sync-all sync-ci-unit lint test-gold-bootstrap test-gold-transition-available test test-ci test-ci-unit test-ci-integration bootstrap up down logs ps migrate-route-cancellation migrate-route-dismiss-restore migrate-route-restore-uniqueness seed bootstrap-gold-seeds seed-e2e e2e-preflight e2e-smoke
...
# 기존에 안내한 타깃도 두 migration을 모두 적용하도록 호환성을 유지한다.
...
.PHONY: deploy-env deploy-secrets deploy-db-bootstrap deploy-db-check \
		deploy-migrate-route-dismiss-restore deploy-migrate-route-restore-uniqueness deploy-seed-models \
...
# 작업 삭제·되돌리기 스키마와 중복 방지 index를 RDS에 순서대로 적용한다.
# **새 API 코드를 올리기 전에** 실행해야 한다. 두 migration은 재실행해도 안전하다.
