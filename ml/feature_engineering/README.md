...
**이 폴더의 pandas 코드(1차 정제 + 옛 2차정제 구현)는 전부 `feature_engineering/legacy/`에
있고, 본 서비스 저장소에는 필요 없다.** 실제 배포에서는 1차 정제(원본 CSV/parquet
→ station_master/targets/status/weather/population) 자체를 이 저장소 밖의 다른
시스템이 처리한다(history.md 5번 항목) — `feature_engineering`은 그 산출물이
`data/processed_v2/*`에 이미 있다고 가정하고 **2차 정제(피처마트 생성)만
Spark로** 담당한다.

| | 위치 | 실행 환경 | 역할 |
|---|---|---|---|
| **2차 정제(Spark) — 본 서비스** | `feature_engineering/spark/*.py` | `feature_engineering/.venv`(uv, Python 3.11, 로컬은 `local[*]` 단일 노드) 또는 EMR `spark-submit` | 1차 정제 산출물을 병합·feature화해 최종 feature 테이블 생성 |
| **1차 정제 + 옛 2차정제(pandas) — legacy, 로컬 테스트 전용** | `feature_engineering/legacy/*.py` | `feature_engineering/.venv` | 1차 정제 산출물을 로컬에서 재현해 위 Spark 경로를 테스트해볼 입력을 만들 뿐, 본 서비스 경로가 아님 |
...
확인했으므로, 피처엔지니어링(2차정제)은 Spark 코드만 유지한다. `feature_engineering/legacy/`가
`ml_common`(파라미터·경로 계약, `<repo-root>/libs/ml_common/` — `ml/`과 별도로
관리되는 공유 라이브러리)만 참조하고 `spark/`를 import하지 않으므로, Spark
쪽을 EMR에 올릴 때는 `feature_engineering/spark/` + `libs/ml_common/` 디렉터리만 있으면
된다. 분류 근거는 [../LEGACY_AUDIT.md](../LEGACY_AUDIT.md) 참고.

## 세팅

```bash
cd ml/feature_engineering
...
# EMR
spark-submit --deploy-mode cluster feature_engineering/spark/run_pipeline.py
```
...
`feature_engineering/spark/config.py`의 `OUTPUT_ROOT`가 자동으로 분리되므로, 다른 조합을
실험할 때 챔피언 산출물을 덮어쓸 걱정 없이
`ROLLING_EMBARGO_MINUTES=45 ./feature_engineering/.venv/bin/python -m feature_engineering.spark.run_pipeline`처럼
환경변수만 바꿔 실행하면 된다(또는 `ML_PROFILE=embargo45`로 프로필째 교체 —
[ml_common README](../../libs/ml_common/README.md) 참고).

**주의**: Spark 스크립트/테스트는 반드시 `feature_engineering/.venv`(uv, Python 3.11)로
...
```bash
cd ml
./feature_engineering/.venv/bin/python -m feature_engineering.legacy.scripts.run_build_pipeline
```
...
다시 돌리고 싶으면 `python -m feature_engineering.legacy.build_weather`처럼 모듈
하나씩 실행하면 된다.
...
```bash
cd ml
./feature_engineering/.venv/bin/python -m pytest feature_engineering/tests/dev_spark_rolling_parity.py feature_engineering/tests/dev_spark_build_features.py feature_engineering/tests/dev_spark_incremental.py -q
```
...
뺐다 — 필요하면 `./feature_engineering/.venv/bin/python -m pytest feature_engineering/legacy/tests -q`로
개별 실행.
...
`libs/ml_common/paths.py`가 `feature_engineering/spark/config.py`와 정확히 같은 공식
