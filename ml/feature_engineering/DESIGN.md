...
`common.rolling_window_features.lookup_count_at_ticks()`를 쓴다 — "그 tick 이하
...
(`common.rolling_window_features.censored_rolling_counts()`), `features.py`가
...
censoring 윈도우/LightGBM 파라미터 등은 `common/common_config.py`가
`ML_PROFILE` 환경변수(기본 `default`)로 `common/profiles/{이름}.json`을 읽어
제공한다. 개별 환경변수(`ROLLING_EMBARGO_MINUTES=45` 등)는 프로필 값 위에
추가로 덮어쓸 수 있다 — 상세는 [common/README.md](../common/README.md).
