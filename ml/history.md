...
- 폴더 구조: `feature-engineering`/`training`/`inference` 분리 예정(지금은
  `feature_engineering/`만 만듦, training/inference는 아직 손 안 댐).
...
## 6. `feature_engineering/` PySpark 패키지

[feature_engineering/](feature_engineering/) — `rolling_window_features.py`(censoring
...
전용 테스트(`test_feature_engineering_build_features.py`)로 검증.
...
(`test_feature_engineering_rolling_parity.py`), 실제 2025년 데이터로 전체 파이프라인
...
**구현**: [feature_engineering/watermark.py](feature_engineering/watermark.py)
...
완전히 일치하는지 대조(`test_feature_engineering_incremental.py`) — 통과.

## 8. `common_config.py` — 공통 설정 분리

**문제**: `src/config.py`(pandas)와 `feature_engineering/config.py`(Spark)가 같은
...
서로 import하지 않는 원칙(EMR엔 `feature_engineering/`만 올라가면 됨)은 유지 —
...
CLI, 기본 dry-run(리포트만), `--execute`로 실제 feature_engineering(Spark, 별도
...
평균"을 담는 조용한 버그였다. (`feature_engineering/build_features.py`가 이미
...
**pandas(`src/`)와 Spark(`feature_engineering/`) 양쪽에 동일하게 적용**
(안 그러면 `test_feature_engineering_rolling_parity.py`가 두 구현의 새로운
...
`src/config.py`/`feature_engineering/config.py`의 인터페이스(`common_config.XXX`
그대로 참조)는 바뀌지 않아야 함.
...
## 15. Spark(`feature_engineering/`) 쪽에 5분 tick 그리드 포팅

**배경**: 10번 항목(pandas `src/`)에서 시간 단위 -> 5분 tick 그리드로 전환했지만
Spark 쪽은 그대로였다. 대칭을 맞추는 작업:

- `feature_engineering/rolling_window_features.py`에 `future_rolling_counts()`
  포팅(pandas와 동일한 차분 배열 기법, `Window.rowsBetween(unboundedPreceding,
  currentRow)`로 누적합).
- `feature_engineering/build_targets.py` 신규 작성 — `src/build_targets.py`의
  Spark 대응. `_normalize_station_no()`를 이 파일에 두고
  `build_rolling_rental_features.py`가 여기서 import하게 정리(pandas 쪽 구조와
  대칭).
- `feature_engineering/build_merged_table.py` 재작성 — station_status를 5분
...
**테스트**: `test_feature_engineering_rolling_parity.py`에 `future_rolling_counts`
pandas/Spark 대조 테스트 추가. `test_feature_engineering_incremental.py`는 손으로
...
`test_feature_engineering_build_features.py`는 그리드 밀도와 무관한 헬퍼라 변경
불필요.

**발견한 중요 버그(이 세션에서 실제로 걸림)**: `feature_engineering/build_targets.py`를
...
해법**: `feature_engineering/rolling_window_features.py`에 `_unix_seconds_ntz()`/
...
`feature_engineering/spark_session.py`와 세 feature_engineering 테스트 fixture
...
tests/test_feature_engineering_*`(12개) 전부 통과, `.venv/bin/python -m pytest
tests/`(45개, feature_engineering 제외) 전부 통과 — 프로필 시스템과 Spark 포팅
