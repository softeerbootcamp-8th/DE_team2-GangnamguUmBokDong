...
있으며, 이미 반납 챔피언이 있으면 학습 시작 전에 실패해 절대 덮어쓰지 않는다.
날짜/프로필은
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """반납 모델 학습 CLI 인자를 파싱한다."""
    parser = argparse.ArgumentParser(description="반납 수요 모델을 학습해 새 아카이브에 저장합니다")
    parser.add_argument(
        "--promote-if-no-champion",
        action="store_true",
        help="반납 챔피언이 아직 없을 때만 학습 결과를 최초 챔피언으로 승격합니다",
    )
...
def main(promote_if_no_champion: bool = False) -> dict:
    ...
    if promote_if_no_champion:
        ensure_champion_absent("return")

    metrics = train_target("return_count", "return", exposure_col=None, models_prefix=models_prefix)
    if promote_if_no_champion:
        bootstrap_challenger("return", models_prefix)
        print(f"[train_return_model] 최초 챔피언 승격 완료: {models_prefix}")
