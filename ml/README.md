...
| **[feature_engineering/](feature_engineering/README.md)** | station×5분tick feature 테이블 생성(Spark, EMR/로컬 `local[*]` 단일 노드) — **본 서비스 코드는 `spark/`뿐**. pandas 1차/2차정제는 전부 `legacy/`(로컬 테스트 입력 준비용, `LEGACY_AUDIT.md` 참고) | `feature_engineering/.venv`(uv, Python 3.11)/EMR |
...
(cd feature_engineering && uv sync)
(cd training && uv sync)
(cd inference && uv sync)

# 1) 데이터셋 생성 (feature_engineering/README.md)
# 1차 정제는 실제 배포에선 이 저장소 밖에서 처리된다 — 로컬에서 처음부터 테스트해보려면
# legacy pandas로 입력을 준비: ./feature_engineering/.venv/bin/python -m feature_engineering.legacy.scripts.run_build_pipeline
./feature_engineering/.venv/bin/python -m feature_engineering.spark.run_pipeline   # 본 서비스 경로(2차정제, local[*] 단일 노드)
...
`feature_engineering/legacy/`로 이동한 옛 pandas 2차정제 구현의 테스트(`dev_features_rental_censoring.py`/
`dev_completion_curve_integration.py`)는 더 이상 기본 검증에 포함하지 않는다 — 여전히
`./feature_engineering/.venv/bin/python -m pytest feature_engineering/legacy/tests -q`로 개별 실행
...
Spark 관련 테스트는 반드시 `feature_engineering/.venv`(uv, Python 3.11)로 실행할 것 —
...
| **[feature_engineering/README.md](feature_engineering/README.md)** / **[DESIGN.md](feature_engineering/DESIGN.md)** | 데이터 파이프라인: 실행 방법 / 설계 배경 |
...
| **[REALTIME_FEATURES.md](REALTIME_FEATURES.md)** | point-in-time 대여 카운트 설계(train-serving skew 대응) — 지금도 유효, 경로만 `feature_engineering`/`ml_common`/`inference`로 갱신해서 읽을 것 |
...
- **생활인구는 250m 격자 기준** — 격자 ID를 좌표로 역산해 정류소와 직접 매칭 (`feature_engineering/legacy/grid.py`, 1차정제 전용).
