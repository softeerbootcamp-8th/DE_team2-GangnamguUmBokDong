...
_SCHEMA_VERSION = "resource-profile-v1"
...
def _process_tree_memory(root_pid: int) -> dict[str, int]:
    """대상 process tree의 현재 RSS·swap과 관측 가능한 process 수를 합산한다."""
    rss = 0
    swap = 0
    observed = 0
...
def _sample_manifest(
    ...
    *,
    ...
    """현재 process/system/filesystem 값을 manifest의 누적 peak에 반영한다."""
    ...
    *,
    ...
    try:
        while process.poll() is None:
            ...
            available = (
                manifest["system_memory_total_bytes"]
                - manifest["peak_system_memory_used_bytes"]
            )
            if (
                minimum_system_memory_available_bytes
