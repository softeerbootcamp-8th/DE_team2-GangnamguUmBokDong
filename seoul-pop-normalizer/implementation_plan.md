...
**이 작업은 `collector`와 완전히 분리된 독립 모듈 `seoul-pop-normalizer/`(브랜치명과 동일, 저장소 루트에 이미 빈 디렉터리로 존재)에 구현한다.** `collector`, `apps/api`, `ml/feature_engineering` 등과 동일하게 `Makefile`의 `PROJECTS`에 등록된 독자적인 uv 프로젝트로 취급하고, `collector`의 내부 코드(스토리지·검증·파이프라인)를 직접 import하지 않는다. **`ml/` 모듈은 이번 설계에서 전례·참고 대상으로 삼지 않는다** — 완전히 독립된 별개 모듈이다.
