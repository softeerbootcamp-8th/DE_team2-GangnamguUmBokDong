...
| `stage>=bronze_written` & 누락 존재 & `--backfill` | 기존 bronze 유지, 누락 조각만 fetch, 전체 재처리, `revision` +1 |
...
주의:
- `attempt` 증가는 manifest 모듈이 담당한다. `revision`은 silver를 실제로 쓴
  경우에만 올리며 `attempt`(실행 횟수)와 다르다.
