"""공통 정책 함수 구현체 
이슈가 발견된 값을 어떻게 처리할지를 담은 함수 모음이다. 
특정 소스에 대한 지식이 들어가는 순간 소스가 늘어도 공통 코드는 바뀌지 않는다는 목표가 깨진다. 
소스별 차이는 전부 YAML(4분면 기본값 + 컬럼별 오버라이드)로 표현한다.
...
class IssueCountParams(BaseModel):
    ...
    `extra="forbid"`가 핵심: loader가 이 모델로 config의 `row_params`를
