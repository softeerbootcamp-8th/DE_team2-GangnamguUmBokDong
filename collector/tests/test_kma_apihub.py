...
def test_fetch_logs_elapsed_for_each_http_request(caplog):
    ...
    assert "base_date=20260812" in request_logs[0]
    assert "base_time=1400" in request_logs[0]
...
def test_fetch_logs_elapsed_on_request_error(caplog):
    ...
    assert "base_date=20260812" in request_log
    assert "base_time=1400" in request_log
...
def test_adjust_base_time_hourly():
    ...
    assert release_base_time(dt, "hourly") == datetime(2026, 8, 12, 13, 0, tzinfo=KST)

    # 40분 이후는 현재 시각 (14:40 -> 14:00)
    dt = datetime(2026, 8, 12, 14, 40, tzinfo=KST)
    assert release_base_time(dt, "hourly") == datetime(2026, 8, 12, 14, 0, tzinfo=KST)


def test_adjust_base_time_half_hourly():
    # 30분 전에는 이전 시각 30분 (14:29 -> 13:30)
    dt = datetime(2026, 8, 12, 14, 29, tzinfo=KST)
    assert release_base_time(dt, "half_hourly") == datetime(
...
def test_adjust_base_time_vilage_fcst():
    ...
    assert release_base_time(dt, "vilage_fcst") == datetime(
