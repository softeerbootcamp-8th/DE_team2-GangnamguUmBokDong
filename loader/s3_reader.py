...
def _predictions_key(window_start: datetime) -> str:
    ...
    `libs/ml_core/silver_schema.py:predictions_key()`와 동일한 형식이지만, 이
    저장소의 기존 컨벤션(loader는 collector/storage.py의 키 규칙도 import 없이
    자체 재구현한다)을 따라 여기서도 독립적으로 재구현한다 — ml_core은 lightgbm
    등 무거운 런타임 의존성을 끌고 오는데 얻는 건 이 한 줄뿐이라 트레이드오프가
    맞지 않는다. 두 구현이 갈라지지 않는지는 dev-only 계약 테스트
    (tests/test_s3_reader.py)가 ml_core.silver_schema.predictions_key()와 직접
    비교해 검증한다.
    """
