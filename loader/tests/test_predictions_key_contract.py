"""dev-only 계약 테스트: loader가 자체 재구현한 predictions 키 컨벤션이
libs/ml_core/silver_schema.py:predictions_key()와 갈라지지 않는지 확인한다.

loader 런타임(main.py/s3_reader.py)은 ml_core을 import하지 않는다 — 이 파일만
예외적으로 dev 의존성(pyproject.toml dependency-groups.dev)의 ml_core을 테스트
목적으로만 가져온다.
"""
