...
| 2 | `feature/collector-config-loader` | config 스키마, YAML 로드, 정책 이름·`row_params` 검증, SHA-256 해시 | `config/schema.py`<br>`config/loader.py` | 1 |
...
| 4 | `feature/collector-storage-manifest` | 경로 규칙, bronze 조각 쓰기·읽기·정리, MinIO 입출력, 상태 어휘와 manifest 스키마 | `storage.py`<br>`manifest.py` | 1 |
| 5 | `feature/collector-validation-engine` | 판정 3단계, 4분면 정책 디스패치, 행 정책, 결과 집계 | `validation/engine.py` | 2·3 |
| 6 | `feature/collector-seoul-adapter` | 어댑터 프로토콜(조각 `yield`)과 서울 열린데이터광장 어댑터 (`RESULT.CODE` 검사·페이지네이션·재시도) | `adapters/base.py`<br>`adapters/seoul_openapi.py` | 2 |
| 7 | `feature/collector-pipeline` | 조각 즉시 저장 루프를 포함한 fetch→bronze→validate→silver 오케스트레이션과 bronze 재사용 재개 분기 | `pipeline.py` | 4·5·6 |
| 8 | `feature/collector-cli-logging` | CLI 인자 파싱, 구조화 로그 고정 필드 주입 | `main.py`<br>`logging_setup.py` | 7 |
...
[구현 계획 10절](./implementation-plan.md#10-구현-순서)의 8단계를 10개로 나눴다. 의존성 추가를 앞으로 뺐고(#2·#3·#4를 병렬로 돌리려면 먼저 병합돼야 한다), 8단계의 "기상청 어댑터 + YAML 7종"은 성격이 달라 #9·#10으로 분리했다.
...
#1 deps
 ...
 └─ #2 ─ #6 seoul ─────────────┘                        ├─ #10 configs
                  └─ #9 kma ─────────────────────────────┘
```

#1 병합 직후 #2·#3·#4를 나눠 잡는 것이 가장 빠르다. #2~#5는 네트워크와 S3 없이 순수 단위 테스트로 끝난다.

---

## 완료 기준

각 이슈는 [구현 계획 11절](./implementation-plan.md#11-검증-방법)의 해당 단위 테스트를 통과하면 완료로 본다.
