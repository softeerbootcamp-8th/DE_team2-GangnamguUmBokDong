...
- ✅ **페이지 행 수를 전체 건수로 오인하던 문제를 수정했다.** `pagination: probe`가
  `list_total_count`를 무시하고 빈 페이지까지 탐색하므로 운영 대여소 약 2,736곳을
  모두 수집한다. 원인·재시도/상한 계약은 **5-18**을 보라. 위의 "표본 1,000건"은
  수정 전 수집이 첫 페이지에서 끊겼을 때의 실측이며 전체 모집단을 뜻하지 않는다.
...
### 5-18. `bike_station_realtime` — 대여소의 63.4%를 매 tick 놓친다 ✅ 수정
...
**수정 내용.** `bike_station_realtime.yaml`에 `pagination: probe`와
`max_probe_pages: 10`을 선언했다. 이 모드는 `list_total_count`를 파싱조차 하지 않고
고정 폭 페이지를 **빈 row 또는 `INFO-200`이 나올 때까지** 순차 요청한다. 종료 응답은
bronze 조각이 아니므로 `persist=False` 메타데이터로만 흘려보내고, 직전 데이터 페이지의
시작 위치와 실제 row 수로 `expected_total`을 계산한다. 따라서 위 실측이면 데이터 조각
3개, `expected_total=2736`이 된다.

라운드 재시도에서는 확정한 `expected_total`로 같은 고정 폭 키를 복원해 성공 조각은
skip하고 실패 조각만 요청한다. 마지막 데이터 페이지나 종료 probe 자체가 일시 실패한
경우도 다음 라운드에서 끝 위치를 다시 확인한다. 빈 끝이 오지 않는 API 이상은 최대
10개 데이터 페이지(10,000행) 뒤의 경계 probe에서 `PERMANENT` 누락으로 실패시켜 조용한
절단을 막는다. 다른 서울 API는 기본 `pagination: total` 경로를 그대로 쓴다.

종료 재시도 사이 snapshot이 축소돼 이미 저장한 page 범위보다 작은 total이 나오거나,
정규화한 Bronze 행 수가 새 total을 초과하면 Silver를 쓰지 않고 fetch 실패로 닫는다.
다만 제공 API에 snapshot token이 없어 **행 수가 같은 재정렬**이나 재시도 사이의 증가를
완전히 원자적으로 탐지할 수는 없다. transient가 하나라도 있으면 전체 probe batch를
새로 받는 정책과 `stationId` 유일성·active-set coverage gate를 함께 설계하는 후속
이슈/PR로 분리한다. 현재 수정은 정적 정상 경로와 명백한 shrink 혼합만 fail-closed한다.

**별도 downstream 계약(미해결).** master 3,428개는 폐쇄·휴업·미운영 후보까지 포함하는
식별자 차원이고, realtime 약 2,736개는 현재 API가 반환하는 운영 snapshot이다. 따라서
둘의 차집합 약 692개에 `rackTotCnt`가 없는 것은 collector 누락이 아니다. 보강 master를
master 전체로 만들면서 realtime capacity를 LEFT JOIN하면 이 행들은 capacity null이 되어
전체 대여소 추론의 부분 실패를 반복한다. 안전한 계약은 보강 차원에는 `is_active`를 두고,
5분 서빙 대상·capacity는 **해당 tick realtime과의 교집합**으로 한정하는 것이다. 과거 학습
식별자 보존이 필요하면 all-master dimension과 active operational snapshot을 별도 산출물로
분리해야 하며, master 행을 임의 삭제해서 과거 트립까지 잃어서는 안 된다.
