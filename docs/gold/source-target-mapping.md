...
- 재고 입력은 기존 `read_recent_stock(anchor, lookback_minutes=25)`의 시간 의미를 그대로
  고정한다. `stock_history_manifest_m25`, `m20`, `m15`, `m10`, `m05`는 각각
  anchor-25·20·15·10·5분의 authoritative source snapshot이다. 현재 anchor는 이미
  commit된 `stock_publication_manifest`가 소유한다. 수집되지 않은 과거 window는
  생략할 수 있지만 최소 2개가 필요하고, publisher가 생략한 offset의 authority가 실제로
  없는지 검증한다. `stock_window_count`는 사용한 과거 window와 현재 1개를 합친 수이며,
  `stock_history_offsets`는 실제 사용한 offset을 oldest-first로 기록한다.
  `scoring_config_version`은 `urgency-scoring-v1`이다. 사용한 source snapshot 전체가
  완전하다면 신규 station이 과거 일부 window에 없을 수는 있지만 PARTIAL snapshot이나
  offset 중복·순서 교환은 허용하지 않는다.
