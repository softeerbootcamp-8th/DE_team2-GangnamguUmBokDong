...
`route-v5-supply-led-mixed-fleet` publisher는 `station_urgency` publication manifest와 현재
Gold topology를 입력으로 사용한다. 센터별 최고 supply urgency가 경로 순서를 소유하고
`center→pickup→supply` 총거리로 안전한 pickup을 고른다. 20대 차량 2대와 15대 차량 2대가
센터별 경로 수와 적재량을 직접 정하며, 별도 stop 수 상한은 두지 않는다. 모든 pickup
뒤 최고 supply를 첫 dropoff로 두어 결정적 UUID와 stop 순서를 만들고 proposed route와
stop을 원자 게시한다. 양수 공급 후보는 현재 빈 공간 안에서 20% 작업 발생 기준보다
40% 방문 회복 목표까지 넉넉히 배분하며, 0수량 후보와 회수 수량은 바꾸지 않는다.
경로는 센터 왕복·이동·정차·상하차를 합쳐 120분 안에 끝나는 가장 큰 완결 이동량을
선택한다.
...
혼합 차량 tuple·버전은 route input fingerprint에 남긴다.
