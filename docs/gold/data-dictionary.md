...
검증한다. 한 pickup station은 같은 plan의 한 경로에서만 사용하며, active route가 이미
예약했거나 cooldown ID 집합에 든 station은 새 pickup 후보에서 제외한다.
