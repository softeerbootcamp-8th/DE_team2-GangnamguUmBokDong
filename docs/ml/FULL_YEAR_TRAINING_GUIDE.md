...
```bash
cd ml
(cd feature_engine && uv sync)   # PySpark, EMR 대상 — 로컬은 local[*]
...
```bash
SPARK_SHUFFLE_PARTITIONS=64 ./feature_engine/.venv/bin/python -m feature_engine.spark.run_pipeline
```

로컬 개발 전용 옵션(`SPARK_MASTER`, `spark.driver.memory` 기본 8g)은
`feature_engine/spark/spark_session.py` 참고 — EMR 등 클러스터 환경에서는
