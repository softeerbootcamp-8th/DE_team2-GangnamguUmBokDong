...
Feature Engine, Training, Airflow 전체 테스트를 실행하여 기능 및 성능 무결성을 검증했다:

```bash
# 1. Feature Engine 파이프라인 테스트 (70 passed)
$ ./feature_engine/.venv/bin/python -m pytest feature_engine/tests -q
70 passed, 443 warnings in 112.57s

# 2. Training 및 모니터링 테스트 (184 passed)
$ ./training/.venv/bin/python -m pytest training/tests -q
184 passed, 1431 warnings in 23.80s

# 3. Airflow DAG 무결성 테스트 (201 passed)
$ ./airflow/.venv/bin/python -m pytest airflow/tests -q
201 passed in 0.61s
```
...
`ml/feature_engine/` 코드는 Airflow 컨테이너가 아니라 **EMR 클러스터**에서 실행된다. EMR은 S3의 `s3://<bucket>/emr/pyfiles.tar.gz`를 부트스트랩 시점에 내려받아 사용한다.

따라서 EC2 운영 환경에 반영할 때는 반드시 **`make emr-package`**를 수행해야 한다:
...
이후 새로 기동되는 EMR 클러스터부터 수정된 최적화 코드가 자동으로 적용된다.
