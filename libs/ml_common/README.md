...
`ml/make_dataset`/`ml/training`/`ml/inference`가 서로 다른 인스턴스에서 따로
...
**이 프로젝트는 `ml/`과 별도로 관리되는 독립 라이브러리다** — `<repo-root>/lib/ml_common/`에
있고, `ml/`의 형제(sibling) 디렉터리다(같은 `lib/` 아래에 다른 서비스의 공유
라이브러리가 더 생길 수 있어서 `common`이 아니라 `ml_common`으로 이름을 붙였다).
`ml/make_dataset`/`ml/training`/`ml/inference`는 각자의 `pyproject.toml`에서
`ml_common`을 editable path 의존성으로 참조한다(`uv sync`로 설치).

이 폴더는 순수 pandas 로직(+ 일부 lightgbm)만 담는다 — `ml/make_dataset/spark/`는
...
```bash
cd ml
ML_PROFILE=default ./make_dataset/.venv/bin/python -m make_dataset.spark.run_pipeline
```
...
(`ml/make_dataset/spark/spark_session.py` 참고). 초 단위 정수 ↔ 타임스탬프
왕복이 필요한 Spark 코드는 `F.unix_timestamp()`/`F.timestamp_seconds()`를 직접
쓰지 말고 `ml/make_dataset/spark/rolling_window_features.py`의
...
```bash
cd ml
./training/.venv/bin/python -m pytest ../lib/ml_common/tests/ -q
```

`ml_common` 자체는 별도 `.venv`를 두지 않는다 — `training`/`inference`/`make_dataset`
