...
`TRAIN_ANCHOR_TICK_MINUTES`는 입력 피처 의미 자체는 바꾸지 않지만 현재 학습
metrics가 각 모델 자신의 anchor 행에서 계산돼 서로 다른 밀도끼리 직접 비교할 수
없다. 공통 5분 evaluator가 생기기 전에는 자동 승격과 runtime을 같은 anchor 계약으로
묶어 잘못된 교차 비교를 fail-closed한다.
"""
