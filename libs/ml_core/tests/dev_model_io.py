...
def test_logs_to_mlflow_only_when_requested(monkeypatch):
    calls = []
    monkeypatch.setattr(model_io.mlflow, "log_artifact", lambda path, artifact_path=None: calls.append((path, artifact_path)))

    model_io.stage_and_upload_booster(_FakeBooster(), "models/test/booster2.txt", log_to_mlflow=True)

    assert len(calls) == 1
    path, artifact_path = calls[0]
    assert artifact_path == "models"
    # NamedTemporaryFile의 난수 이름(tmpXXXXXX.txt)이 아니라 S3 키의 마지막 조각
    # ("booster2.txt")과 같은 이름으로 업로드돼야 MLflow UI에서 식별 가능하다
    # (2026-08 리뷰 지적).
    assert Path(path).name == "booster2.txt"
