"""minute_of_day()가 자정 기준 경과분을 정확히 계산하는지, tick(20분) 단위로
...
def test_distinguishes_ticks_within_same_hour():
    """17:00/17:20/17:40은 hour로는 구분이 안 되지만 minute_of_day는 셋 다 달라야 한다."""
    values = {minute_of_day(pd.Timestamp(f"2025-06-01 17:{m:02d}:00")) for m in (0, 20, 40)}
    assert len(values) == 3
