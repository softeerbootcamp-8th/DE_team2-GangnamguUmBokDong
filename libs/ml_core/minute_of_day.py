"""시각을 자정 기준 경과분(0~1439) 하나로 표현하는 `minute` 피처다.

`hour`만 쓰면 같은 시간 안의 서로 다른 모델 tick(기본 20분의
17:00/17:20/17:40, 5분 비교 실험의 더 촘촘한 시점)이 전부 같은 값으로 보인다.
`hour*60+minute`(00:00=0, 01:20=80, ...)은 하루 안에서 단조증가하며 tick 단위
구분을 그대로 담는다.
